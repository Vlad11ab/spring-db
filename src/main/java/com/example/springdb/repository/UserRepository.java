package com.example.springdb.repository;

import com.example.springdb.model.UserApp;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.Pageable;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.data.jpa.repository.JpaSpecificationExecutor;
import org.springframework.data.jpa.repository.Query;
import org.springframework.data.repository.query.Param;
import org.springframework.stereotype.Repository;

import java.time.LocalDate;
import java.util.List;
import java.util.Optional;

@Repository
public interface UserRepository extends JpaRepository<UserApp,Long> , JpaSpecificationExecutor<UserApp> {

    Optional<UserApp> findUserByEmail(String email);

    @Query("select u from UserApp u where lower(u.firstName) = lower(:firstName)")
    Optional<UserApp> findByFirstNameIgnoreCaseJPQL(@Param("firstName")String firstName);

    @Query("select u from UserApp u where lower(u.lastName) = lower(:lastName)")
    Optional<UserApp> findByLastNameIgnoreCaseJPQL(@Param("lastName") String lastName);

    @Query("select u from UserApp u where lower(u.email) = lower(:email)")
    Optional<UserApp> findByEmailIgnoreCaseJPQL(@Param("email") String email);

    @Query("select u from UserApp u\n"+
            "where u.age between :minAge and :maxAge\n" +
            "order by u.age asc")
    List<UserApp> findByAgeRange (@Param("minAge") int minAge,
                                  @Param("maxAge") int maxAge);

    @Query("select u from UserApp u\n" +
            "where u.hireDate >= :from and u.hireDate <= :to\n" +
            "order by u.hireDate asc")
    List<UserApp> findHiredBetween(@Param("from") LocalDate from,
                                   @Param("to") LocalDate to);

    @Query("select u from UserApp u\n" +
            "where lower(u.firstName) like lower(concat('%', :q, '%'))" +
            "or lower(u.email) like lower(concat('%', :q, '%'))" +
            "or u.phoneNumber like concat('%', :q, '%')")
    Page<UserApp> searchByFirstNameOrEmailOrPhoneNumber(@Param("q") String q, Pageable pageable);


    @Query("select u from UserApp u where" +
            "(u.firstName is null or u.firstName=:firstName) and" +
            "(u.lastName is null or u.lastName=:lastName) and " +
            "(u.email is null or u.email=:email) and" +
            "(u.age is NULL or u.age=:age) and" +
            "(u.hireDate is null or u.hireDate=:hireDate) and " +
            "(u.password is null or u.password=:password) and" +
            "(u.phoneNumber is null or u.phoneNumber=:phoneNumber)")
    List<UserApp> search(
            @Param("firstName") String firstName,
            @Param("lastName") String lastName,
            @Param("email") String email,
            @Param("age") int age,
            @Param("hireDate") LocalDate hireDate,
            @Param("password") String password,
            @Param("phoneNumber") String phoneNumber);

    @Query("select count(u) from UserApp u where u.hireDate <= :date")
    long countHiredBefore(@Param("date") LocalDate date);

    @Query("select (count(u) > 0) from UserApp u where lower(u.email) = :email")
    boolean existsByEmailJPQL(@Param("email") String email);

    @Query("select (count(u) > 0) from UserApp u where lower(u.phoneNumber) = :phoneNumber")
    boolean existsByPhoneNumber(@Param("phoneNumber") String phoneNumber);
}

